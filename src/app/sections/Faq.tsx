...
export default function Timeline({ id }: { id?: string }) {
  ...
  const getStatusBadge = (status: ModuleStatus) => {
    ...
    const badges = {
      upcoming: {
        text: "Незабаром почнеться",
        className:
          "bg-primary text-white font-medium uppercase text-xs px-2 py-0.5 rounded-sm",
      },
      current: {
        text: "Триває",
        className:
      ...
      next: {
        text: "Наступний",
        className:
          "text-primary border border-dashed font-medium uppercase text-xs px-2 py-0.5 rounded-sm",
      },
    };
  ...
  return (
    <section id={id || "timeline"} className="relative w-full min-h-screen py-12 md:py-20">
      <motion.div
        className="max-w-6xl mx-auto text-center text-black px-6 md:px-4"
      ...
      <div className="max-w-7xl mx-auto font-mono my-16 px-4 sm:px-0">
        <Accordion type="single" collapsible>
          {modulesWithStatus.map((module, i) => (
            <motion.div
              key={module.id}
